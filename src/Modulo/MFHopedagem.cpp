#include <curse.h>
#include "MFHospedagem.hpp"

MFHospedagem::run(Conta* conta){
    initscr();
    noecho();
    cbreak();
    int resposta = 0;
...
MFHospedagem::setCntrIBHospedagem(IBHospedagem* ibHospedagem){
    this->ibHospedagem=ibHospedagem;
}

MFHospedagem::~IFHospedagem(){
    ibHospedagem=nullptr;
}
